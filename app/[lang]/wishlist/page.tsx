...
import { BasketPageLayout, WishlistPageLayout } from "@/src/layout";
...
export async function generateMetadata({ params: { lang } }: { params: { lang: LocaleType } }): Promise<Metadata> {
    try {
        ...
        const pageTitle = `${titleDictionary.sobsan} | ${titleDictionary.wishlist}`;
        return {
            title: pageTitle
        };
    } catch (error) {
...
const WishlistPage = async ({ params: { lang } }: { params: { lang: LocaleType } }) => {
    try {
        const t = await getTranslate(lang);
        const titleDictionary = t.title;
