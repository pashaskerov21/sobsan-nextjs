...
import { WarrantyConditionsPageLayout } from '@/src/layout';
...
export async function generateMetadata({ params: { lang } }: { params: { lang: LocaleType } }): Promise<Metadata> {
  ...
  const pageTitle = `${titleDictionary.sobsan} | ${titleDictionary.warranty_conditions}`;
  return {
    title: pageTitle
  };
}

const WarrantyConditionsPage = async ({ params: { lang } }: { params: { lang: LocaleType } }) => {
  try {
    const t = await getTranslate(lang);
    const titleDictionary = t.title;
