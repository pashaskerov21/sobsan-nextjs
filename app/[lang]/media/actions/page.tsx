...
import { ActionPageLayout } from '@/src/layout';
...
export async function generateMetadata({ params: { lang } }: { params: { lang: LocaleType } }): Promise<Metadata> {
    try {
        ...
        const pageTitle = `${titleDictionary.sobsan} | ${titleDictionary.actions}`;
        return {
            title: pageTitle
        };
    } catch (error) {
...
const ActionsPage = async ({ params: { lang } }: { params: { lang: LocaleType } }) => {
    try {
        const t = await getTranslate(lang);
        const titleDictionary = t.title;
